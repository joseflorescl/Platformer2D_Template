...
public class SandboxPlayerMovement : MonoBehaviour
{
    ...
    SandboxCharacterController2D SANDBOX_charactercontroller2D;
    PlayerInput input;
    State currentState;
    ...
    private void Awake()
    {        
        SANDBOX_charactercontroller2D = GetComponent<SandboxCharacterController2D>();
    ...
    private void Start()
    {
        ...
        SANDBOX_charactercontroller2D.SetPhysicsVariables(jumpMaxHeight, jumpTimeToMaxHeight, jumpMinHeight);
    }
    ...
    {
        if (SANDBOX_charactercontroller2D)
            SANDBOX_charactercontroller2D.SetPhysicsVariables(jumpMaxHeight, jumpTimeToMaxHeight, jumpMinHeight);
    }
    ...
    private void FixedUpdate()
    {
        ...
        SANDBOX_charactercontroller2D.PhysicsCheck(fallGravityMultiplier);

        if (SANDBOX_charactercontroller2D.BeginFalling)
            GameManager.Instance.PlayerBeginFalling();

        IsOnGround = SANDBOX_charactercontroller2D.IsOnGround;
        ...
        SANDBOX_charactercontroller2D.MoveWithImpulse(xVelocity, acceleration);
        SANDBOX_charactercontroller2D.ClampMinVerticalSpeed(maxFallSpeed);
    }
    ...
    private void JumpEnter()
    {
        ...
        SANDBOX_charactercontroller2D.JumpWithImpulse();
    ...
    private void JumpUpdate()
    {
        ...
        {
            SANDBOX_charactercontroller2D.ClampMaxVerticalSpeed();
        }
    }
